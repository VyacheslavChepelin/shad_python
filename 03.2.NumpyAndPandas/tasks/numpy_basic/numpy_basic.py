...
def construct_array(
    ...
    :return: matrix slice
    """
    return np.array([matrix[row_indices[ind], col_indices[ind]]
                     for ind in range(max(len(row_indices), len(col_indices)))])
...
def construct_matrix(
    ...
    return np.array([[first_array[ind], second_array[ind]] for ind in range(max(len(first_array), len(second_array)))])
